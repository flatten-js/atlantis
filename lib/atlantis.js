...
class Atlantis {
  ...
  async _on_restricted_area(output, dest) {
  ...
  async uncompress(src, algorithm, key) {
    try {
      ...
      throw new Error(`[${e.code}] Failed to read folder to decrypt`)
    }

    const dest = path.join(path.dirname(src), 'atlantis')
    const input = fs.createReadStream(src)
    const output = unzipper.Extract({ path: dest })
    input.pipe(decrypt(key, algorithm, stat.size)).pipe(output)

    this._on_restricted_area(output, dest)
